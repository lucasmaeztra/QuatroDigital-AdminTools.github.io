...
function gui_row_select(id, setfocus, multiple) {
	...
	// QD - Personalizado
	$(window).trigger('QD_workingArea', id);
}
...
function gui_addArea(id) {
	...
	var temp = '<td>';
	temp+= '<input type="text"  name="img_id" class="img_id" value="' + id + '" readonly="1"/>';
	//temp+= '<input type="checkbox" name="img_active" class="img_active" id="img_active_'+id+'" value="'+id+'">';
	//could be checkbox in the future
	temp+= '</td><td class="hidden">';
	temp+= '<input type="radio" name="img_active" class="img_active" id="img_active_'+id+'" value="'+id+'">';
	temp+= '</td><td>';
	temp+= '<input type="text" name="img_href" class="form-control img_href" value="">';
	temp+= '</td><td>';
	temp+= '<button type="button" onclick="myimgmap.removeArea(' + id + ')" class="btn btn-danger">Remover</button>';
	temp+= '</td><td>';
	// QD - Bloco não utilizado
		temp+= '<select name="img_shape" class="img_shape hidden">';
		...
		// }
		temp+= '</select>';
		temp+= '<input type="text" name="img_coords" class="img_coords hidden" value="">';
		temp+= '<input type="text" name="img_alt" class="img_alt hidden" value="">';
		temp+= '<select name="img_target" class="img_target hidden">';
	...
	myimgmap.addEvent(props[id].getElementsByTagName('input')[3],  'keydown', gui_coords_keydown);
...
function gui_htmlShow() {
	...
	console.log(myimgmap.getMapHTML());
}
...
mode : "editor",
custom_callbacks : {
	// 'onAddArea': function() {console.log('onAddArea')},
	// 'onRemoveArea': function() {console.log('onRemoveArea')},
	// 'onDrawArea': function() {console.log('onDrawArea')}, 
	// 'onStartDrawArea': function() {console.log('onStartDrawArea')}, 
	// 'onDrawedArea': function() {console.log('onDrawedArea')}, 
	// 'onResizeArea': function() {console.log('onResizeArea')},
	// 'onRelaxArea': function() {console.log('onRelaxArea')},
	// 'onFocusArea': function() {console.log('onFocusArea')},
	// 'onBlurArea': function() {console.log('onBlurArea')},
	// 'onMoveArea': function() {console.log('onMoveArea')},
	// 'onSelectRow': function() {console.log('onSelectRow')},
	// 'onSetMap': function() {console.log('onSetMap')},
	// 'onGetMap': function() {console.log('onGetMap')},
	// 'onSelectArea': function() {console.log('onSelectArea')},
	// 'onDblClickArea': function() {console.log('onDblClickArea')},
	// 'onStatusMessage': function() {console.log('onStatusMessage')},
	// 'onAreaChanged': function() {console.log('onAreaChanged')},
	
	'onLoadImage' :  function(img) {
		$(window).trigger('QD_onLoadImage', img); 
	}, 
	'onStatusMessage' : function(str) {  gui_statusMessage(str);},//to display status messages on gui
	'onHtmlChanged'   : function(html) { 
		$(window).trigger('QD_onHtmlChanged', html);  
		gui_htmlChanged(html);
	...
	'onAddArea'       : function(id)  {
		$(window).trigger('QD_newAreaCreated', id);  
		gui_addArea(id);
	},//to add new form element on gui
	'onRemoveArea'    : function(id)  { 
		$(window).trigger('QD_onRemoveArea');  
		gui_removeArea(id);
	},//to remove form elements from gui
	'onAreaChanged'   : function(obj) {  gui_areaChanged(obj);},
	'onSelectArea'    : function(obj) { 
		$(window).trigger('QD_onSelectArea', obj); 
		gui_selectArea(obj);
	},//to select form element when an area is clicked
	
	// QD Personalizado
	'onDrawedArea'    : function(id) { $(window).trigger('QD_onDrawedArea', id); },//to select form element when an area is clicked
	'onRelaxArea'    : function(id) { $(window).trigger('QD_onRelaxArea', id); }//to select form element when an area is clicked
},
