...
imgmap.prototype.getMapHTML = function(flags) {
	...
	// QD - Personalizado
	this.error =  this.getImgSrc().length ? false : "Aba sem imagem.";

	var html = '<map id="'+this.getMapId()+'" name="'+ mapName +'">\n' + this.getMapInnerHTML(flags) + this.waterMark + '</map>';
	html += '\n<img src="' + this.getImgSrc() + '" usemap="#' + mapName + '" />';
...
imgmap.prototype.getMapName = function() {
	if (this.mapname === '') {
		...
		var now = new Date();
		this.mapname = 'imgmap' + now.getMilliseconds() + now.getHours() + now.getMinutes() + now.getSeconds();
	}
	return this.mapname;
};
...
// QD - Personalizado
imgmap.prototype.setImgSrc = function(imgsrc) {
	this.imgsrc = imgsrc;
	return true;
}
imgmap.prototype.getImgSrc = function() {
	return this.imgsrc;
}
 ...
 *	@since	2.1
 */
imgmap.prototype._setBorder = function(id, style, noSetColor) {
	var setColor = noSetColor || false;
...
imgmap.prototype.img_mousedown = function(e) {
	...
	if (this.is_drawing == this.DM_POLYGON_DRAW || this.is_drawing == this.DM_BEZIER_DRAW) {
		...
		// QD Personalizado
		this.fireEvent('onDrawedArea', this.currentid);
	...
	this.memory[this.currentid].downx  = x;
	this.memory[this.currentid].downy  = y;
	this.fireEvent('onStartDrawArea', this.currentid);
};
